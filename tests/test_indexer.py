...
def file_in_testdir(filename):
    return os.path.join("/Users/cgr/git/mpy_dev/tests", filename)
...
def test_fieldset_select_single_file():
    return
    f = FieldList(paths=file_in_testdir("tuv_pl.grib"))
    assert f._indexer is None
    ...
    assert len(g) == 1
    assert g.grib_get(["shortName", "level:l"]) == [["u", 700]]
    ...
    assert g._indexer is not None
    md = [{"level": 700, "shortName": "u"}]

    assert md == g._indexer.items
    ...
    assert len(g) == 1
    assert g.grib_get(["paramId:l", "level:l"]) == [[131, 700]]
    # g1 = f[7]
    # d = g - g1
    ...
    assert len(g) == 4
    assert g.grib_get(g, ["shortName", "level:l"]) == [
        ["t", 700],
        ["u", 700],
        ["t", 500],
        ["u", 500],
